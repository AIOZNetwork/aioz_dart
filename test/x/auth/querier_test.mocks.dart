// Mocks generated by Mockito 5.1.0 from annotations
// in aioz/test/x/auth/querier_test.dart.
// Do not manually edit this file.

import 'dart:async' as _i6;

import 'package:aioz/proto/cosmos/auth/v1beta1/query.pb.dart' as _i5;
import 'package:aioz/proto/cosmos/auth/v1beta1/query.pbgrpc.dart' as _i4;
import 'package:grpc/service_api.dart' as _i2;
import 'package:grpc/src/client/call.dart' as _i3;
import 'package:mockito/mockito.dart' as _i1;

// ignore_for_file: type=lint
// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis
// ignore_for_file: camel_case_types

class _FakeResponseFuture_0<R> extends _i1.Fake
    implements _i2.ResponseFuture<R> {}

class _FakeClientCall_1<Q, R> extends _i1.Fake implements _i3.ClientCall<Q, R> {
}

class _FakeResponseStream_2<R> extends _i1.Fake
    implements _i2.ResponseStream<R> {}

/// A class which mocks [QueryClient].
///
/// See the documentation for Mockito's code generation for more information.
class MockQueryClient extends _i1.Mock implements _i4.QueryClient {
  MockQueryClient() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i2.ResponseFuture<_i5.QueryAccountsResponse> accounts(
          _i5.QueryAccountsRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(#accounts, [request], {#options: options}),
              returnValue: _FakeResponseFuture_0<_i5.QueryAccountsResponse>())
          as _i2.ResponseFuture<_i5.QueryAccountsResponse>);
  @override
  _i2.ResponseFuture<_i5.QueryAccountResponse> account(
          _i5.QueryAccountRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(#account, [request], {#options: options}),
              returnValue: _FakeResponseFuture_0<_i5.QueryAccountResponse>())
          as _i2.ResponseFuture<_i5.QueryAccountResponse>);
  @override
  _i2.ResponseFuture<_i5.QueryAccountAddressByIDResponse> accountAddressByID(
          _i5.QueryAccountAddressByIDRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(
                  #accountAddressByID, [request], {#options: options}),
              returnValue:
                  _FakeResponseFuture_0<_i5.QueryAccountAddressByIDResponse>())
          as _i2.ResponseFuture<_i5.QueryAccountAddressByIDResponse>);
  @override
  _i2.ResponseFuture<_i5.QueryParamsResponse> params(
          _i5.QueryParamsRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(#params, [request], {#options: options}),
              returnValue: _FakeResponseFuture_0<_i5.QueryParamsResponse>())
          as _i2.ResponseFuture<_i5.QueryParamsResponse>);
  @override
  _i2.ResponseFuture<_i5.QueryModuleAccountsResponse> moduleAccounts(
          _i5.QueryModuleAccountsRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
          Invocation.method(#moduleAccounts, [request], {#options: options}),
          returnValue:
              _FakeResponseFuture_0<_i5.QueryModuleAccountsResponse>()) as _i2
          .ResponseFuture<_i5.QueryModuleAccountsResponse>);
  @override
  _i2.ResponseFuture<_i5.Bech32PrefixResponse> bech32Prefix(
          _i5.Bech32PrefixRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(#bech32Prefix, [request], {#options: options}),
              returnValue: _FakeResponseFuture_0<_i5.Bech32PrefixResponse>())
          as _i2.ResponseFuture<_i5.Bech32PrefixResponse>);
  @override
  _i2.ResponseFuture<_i5.AddressBytesToStringResponse> addressBytesToString(
          _i5.AddressBytesToStringRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(
                  #addressBytesToString, [request], {#options: options}),
              returnValue:
                  _FakeResponseFuture_0<_i5.AddressBytesToStringResponse>())
          as _i2.ResponseFuture<_i5.AddressBytesToStringResponse>);
  @override
  _i2.ResponseFuture<_i5.AddressStringToBytesResponse> addressStringToBytes(
          _i5.AddressStringToBytesRequest? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
              Invocation.method(
                  #addressStringToBytes, [request], {#options: options}),
              returnValue:
                  _FakeResponseFuture_0<_i5.AddressStringToBytesResponse>())
          as _i2.ResponseFuture<_i5.AddressStringToBytesResponse>);
  @override
  _i3.ClientCall<Q, R> $createCall<Q, R>(
          _i2.ClientMethod<Q, R>? method, _i6.Stream<Q>? requests,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
          Invocation.method(
              #$createCall, [method, requests], {#options: options}),
          returnValue: _FakeClientCall_1<Q, R>()) as _i3.ClientCall<Q, R>);
  @override
  _i2.ResponseFuture<R> $createUnaryCall<Q, R>(
          _i2.ClientMethod<Q, R>? method, Q? request,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
          Invocation.method(
              #$createUnaryCall, [method, request], {#options: options}),
          returnValue: _FakeResponseFuture_0<R>()) as _i2.ResponseFuture<R>);
  @override
  _i2.ResponseStream<R> $createStreamingCall<Q, R>(
          _i2.ClientMethod<Q, R>? method, _i6.Stream<Q>? requests,
          {_i2.CallOptions? options}) =>
      (super.noSuchMethod(
          Invocation.method(
              #$createStreamingCall, [method, requests], {#options: options}),
          returnValue: _FakeResponseStream_2<R>()) as _i2.ResponseStream<R>);
}
